...
export const fetchComments = async ({skip=0, blog_id, setParentCommentCountFun, comment_array = null}) => {
    ...
    await axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/comment/get-comments", {blog_id, skip})
    .then(({data}) => {
        data.map(comment => {
        ...
        <div className={"max-sm:w-full fixed " +(commentsWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]" ) +" duration-700 max-sm:right-0 max-sm sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
...

            {
                ...
                commentsArr.map((comment, idx) => {
                    return <AnimationWrapper key={idx}>
                        <CommentCard index={idx} leftVal={comment.childrenLevel * 4} commentData={comment} />
                    </AnimationWrapper>;
                })
                :
