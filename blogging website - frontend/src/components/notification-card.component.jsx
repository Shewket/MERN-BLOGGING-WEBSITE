...
const NotificationCard = ({data, index, notificationState}) => {
    ...
    const handleDelete = (type_id, type, target) => {

        target.setAttribute("disabled", true);

        axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/comment/delete-comment", {_id: type_id}, {
            headers: {
            ...
            <div className="flex gap-5 mb-3">
                ...
                <div className="w-full">
                    <h1 className="font-medium text-xl text-dark-grey">
                        ...
                        <Link to={`/user/${username}`} className="mx-1 text-black">@<span className="underline">{username}</span></Link>
...

            {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    <div className="flex gap-3 mb-3">
                        ...
                        <div>
                            <h1 className="font-medium text-xl text-dark-grey">
                                <Link to={`/user/${author_username}`} className="mx-1 text-black">@<span className="underline">{author_username}</span></Link>
