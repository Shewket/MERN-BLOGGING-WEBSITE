        ...
        <div className="flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey max-lg:border-b">
            {
                Object.keys(status).map((key, idx) => {
                    ...
                    <div key={idx} className={"flex flex-col items-center w-full h-full justify-center p-4 px-6 " + (idx != 0 ? "border-grey border-l " : "")}>
                        <h1 className="text-xl lg:text-2xl mb-2">{status[key].toLocaleString()}</h1>
                        <p className="max-lg:text-dark capitalize">{key.split("_")[1]}</p>
                    </div> : ""
                })
            }
...
const deleteBlog = (blog, access_token, target) => {
    ...
    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/blog/delete-blog", {blog_id}, {
        headers: {
    ...
    return (
        <>
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <div className="flex flex-col justify-between py-5 w-full min-w-[300px]">
                    <div>
                    ...
                    <div className="flex gap-6 mt-3">
                        ...
                        <button className="lg:hidden pr-4 py-2 underline" onClick={() => setShowStat(preVal => !preVal)}>Status</button>
                ...
                <div className="max-lg:hidden">
                    <BlogStatus status={activity} />
                </div>
            </div>

            {
                showStat ? <div className="lg:hidden"><BlogStatus status={activity} /></div>
                : ""
            }
        </>
    )
